...
async def quantitative_agent(azure_ai_client: AIProjectClient):
    """
    ...
    """
    agent = await create_quantitative_agent(azure_ai_client=azure_ai_client)
    yield agent
    await azure_ai_client.agents.delete_agent(agent.id)


@pytest.mark.asyncio
async def test_task_with_basic_interpreter(quantitative_agent: AzureAIAgent):
    """
    ...
    response = await quantitative_agent.get_response(messages=task)
...
@pytest.mark.asyncio
async def test_task_with_intermediate_interpreter(quantitative_agent: AzureAIAgent):
    """
    Test the agent with a task that requires basic code generation and execution.
    """
    task = """The company made the following revenue every year: 
...
@pytest.mark.asyncio
async def test_task_with_projection_tool(quantitative_agent: AzureAIAgent):
    """
